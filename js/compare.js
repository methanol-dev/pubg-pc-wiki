    ...
    if (activeWeapons.length < 2) {
      container.innerHTML = `
        <div style="text-align: center; padding: 4rem 1rem; color: var(--text-muted);">
          <p style="font-family: var(--font-display); font-size: 1.25rem;">
            ⚠️ ${window.i18n.t('compare.subtitle', 'Vui lòng chọn ít nhất 2 khẩu súng để bắt đầu so sánh.')}
          </p>
        </div>
      `;
      return;
            ...
            <tr>
              ...
              ${statsList.map((s) => `
                <td class="val-cell ${s.headDamage >= 100 ? 'winner' : ''}">
                  ${s.headDamage} dmg
                  ${s.headDamage >= 100 ? '<div style="font-size: 0.7rem; color: #ff6b6b;">1-Tap Knockout!</div>' : ''}
                </td>
              `).join('')}
            </tr>
          </tbody>
